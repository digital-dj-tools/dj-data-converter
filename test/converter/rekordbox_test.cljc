...
;; TODO why does this fail with spec-tools 10.4+
(defspec-test
  track->item
  `r/track->item
  {:opts {:num-tests 2}})
...
(comment 
  (s/explain u/item-from-rekordbox-spec (second (first (s/exercise-fn `r/track->item 1)))))

(comment 
  (s/explain u/item-spec (second (first (s/exercise-fn `r/track->item 1)))))
